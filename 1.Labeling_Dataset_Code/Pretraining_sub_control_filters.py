...
def loading_paths_from_MAT(Pri_path_file_name, Sec_path_file_name):
    # โหลดไฟล์ .mat
    Pri_dfs = sio.loadmat(Pri_path_file_name)
    Secon_dfs = sio.loadmat(Sec_path_file_name)
    
    # ดึงข้อมูลออกมา (ต้องชื่อตรงกับที่คุณบันทึกไว้ คือ 'Pz1' และ 'S')
...
def main():
    ...
    Pri_path, Secon_path = loading_paths_from_MAT(
        Pri_path_file_name='Primary_Path_WhiteNoise.mat', 
        Sec_path_file_name='Secondary_Path_WhiteNoise.mat' # หรือชื่อที่คุณตั้งไว้ตอนวัด S
    )
